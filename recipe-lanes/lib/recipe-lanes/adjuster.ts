...
- **Merging nodes**: Always do BOTH steps: (1) add the combined result in \`addNodes\`, (2) put every source node in \`removeNodeIds\`. Never remove nodes without adding their replacement. Dangling inputs refs to removed nodes are cleaned automatically — do NOT manually add \`updateNodes\` entries just to clear refs to removed nodes.
