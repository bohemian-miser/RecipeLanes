...
Outputs:
  ...
  scripts/ie_data/image_embed_cache.json     — {id: [float, ...]} (4096-dim pixel vector,
                                               or 512-dim if CLIP is used)
  scripts/ie_data/image_embeddings.npy       — float32 shape (N, embed_dim), row i = item i
  scripts/ie_data/image_embed_method.txt     — notes which method was used (pixel or CLIP)
"""
...
# ---------------------------------------------------------------------------
# Paths — all relative to the recipe-lanes/ working directory
# ---------------------------------------------------------------------------
...
try:
    ...
    print('torch and transformers found — attempting to load CLIP model...')
    ...
    USE_CLIP = True
    print(f'CLIP loaded successfully — running on: {clip_device}')
except ImportError:
    print('torch / transformers not available — using pixel embeddings (4096-dim).')
except Exception as e:
    print(f'CLIP load failed ({e}) — falling back to pixel embeddings.')
...
def make_clip_embed(img: Image.Image) -> np.ndarray:
    """Return a 512-dim CLIP image embedding (L2-normalised)."""
    import torch  # already confirmed importable at this point
    with torch.no_grad():
        inputs = clip_processor(
            images=img.resize((224, 224)),
            return_tensors='pt'
        ).to(clip_device)
        feats = clip_model.get_image_features(**inputs)
        feats = feats / feats.norm(dim=-1, keepdim=True)
        return feats.cpu().numpy()[0]  # shape (512,)
...
# ---------------------------------------------------------------------------
method_str = (
    f'CLIP (openai/clip-vit-base-patch32, device={clip_device}) — 512-dim vectors'
    if USE_CLIP else
