...
# echo "----------------------------------------------------------------"
# npx env-cmd -f .env.test node --import tsx --test tests/*
# # graph.test.ts tests/data.test.ts tests/image-processing.test.ts tests/verify-production-logic.test.ts tests/title-persistence.test.ts tests/icon-search.test.ts tests/parser.test.ts tests/icon-shortlist.test.ts tests/icon-pipeline.test.ts tests/recipe-store.test.ts
...
if curl -s --connect-timeout 1 http://127.0.0.1:8080 > /dev/null 2>&1; then
    ...
    npx env-cmd -f .env.test node --import tsx --test tests/*
    # tests/admin-security.test.ts tests/lifecycle.test.ts tests/functions-metadata.test.ts tests/icon-index.test.ts
else
    ...
    npx env-cmd -f .env.test firebase emulators:exec --only auth,firestore,storage,functions,tasks --project local-project-id "node --import tsx --test tests/*"
    # tests/admin-security.test.ts tests/lifecycle.test.ts tests/functions-metadata.test.ts tests/icon-index.test.ts"
fi
