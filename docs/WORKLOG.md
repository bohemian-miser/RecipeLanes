...
## 2026-09-13 — Gallery "Compare" table: multi-recipe ingredient comparison (signed-in only)

**Feature (owner-directed).** Signed-in users can tick several recipe cards in the gallery (any tab — Gallery / Mine / Starred) and get a table at the top of the page: the selected recipes as **columns** (preview icon + title, linking to the editor), their ingredients as **rows** (icon, label, unit on the left), the per-recipe quantity in each cell, and a **Total** column on the right. Rows and columns are reorderable by drag-and-drop (native HTML5 DnD — no new dependency) and, because native DnD is pointer-only, by arrow keys on the grip handles. The table scrolls inside its own box so the recipe header row and the ingredient column float (a page-level sticky header is impossible: the horizontal-overflow wrapper is the nearest scroll container). The tick is an always-visible labelled "Compare / Comparing" pill — the first cut was a hover-revealed box that was easy to miss and invisible on touch. Guests see neither the tick boxes nor the table.

**Data-quality finding surfaced by this table.** Staging rows like "Of Lamb · rack", "Buns · burger", "Onion, Thinly Sliced · red" and "(400g) Firm Tofu · block" are not bad staging data: the LLM only emits the node `text` ("1 rack of lamb"), and `parseQuantity` in `lib/recipe-lanes/parser.ts` splits it with a regex that treats *any* alphabetic token after the number as the unit. The split is stored on the node at creation, so existing recipes keep it. Fixing it (a unit vocabulary in the parser, plus a backfill or lazy re-parse for stored nodes, and ideally a canonical ingredient identity shared across recipes) is a separate change — tracked as a follow-up, not folded into this PR.

**Where the logic lives.** `lib/recipe-lanes/comparison-table.ts` is pure and owns every merging rule: an ingredient row is keyed by its **displayed label plus unit** (summing "200 g flour" with "1 cup flour" would be meaningless, so units split rows; keying on the node's icon description instead produced two rows that both read "Salt · tsp" on staging, because that field is a per-recipe art prompt, not an ingredient identity), repeated lines inside one recipe are summed, and quantities are scaled to the recipe's current `serves` exactly as `handleUpdateServes` does in the editor. A recipe that lists an ingredient without a number shows `✓` in its cell and marks the row total as partial (`5+`), rather than silently reading as zero. `reconcileOrder` keeps a user-arranged row/column order stable while recipes are added and removed.

**Data path.** New server action `getComparisonRecipesAction(ids)` (login required, capped at `MAX_COMPARISON_RECIPES = 12`) returns the slim `ComparisonRecipe` shape — never the raw graph, so icon shortlists never cross the wire. It runs on the Admin SDK, which bypasses rules, so `canViewRecipeForComparison` re-applies the Firestore `recipes` read rule (public/unlisted, or owner) per id; recipes the caller may not see are dropped and the client unticks them with a notice instead of showing a phantom column.

**Selection state** (`components/recipe-comparison/comparison-context.tsx`) is mirrored to `sessionStorage` per user via `useSyncExternalStore`: the gallery is a server-rendered page and every tab switch remounts it, so without this a cross-tab comparison (one recipe from Mine, one from Starred) would be impossible. The server and first client render both see the empty selection (no hydration mismatch); the stored ticks appear on the next paint. The provider is mounted by `app/gallery/page.tsx` only when a session exists — `useRecipeComparison()` returning null is the "signed out" signal the card toggle uses to render nothing.

**Tests.** `tests/comparison-table.test.ts` (pure tier, auto-discovered): extraction, serves scaling, unit-split rows, within-recipe summing, partial totals, float-noise, order reconciliation, `moveItem`, and the access gate. `e2e/recipe-comparison.spec.ts`: two mock recipes, tick both, assert columns/rows/cells/totals, reorder rows and columns by both drag and keyboard, selection surviving a tab switch, remove/clear, and a guest context seeing no toggles on a published card (new admin helper `publishRecipe` in `e2e/utils/admin-utils.ts` publishes + vets straight through the admin SDK).
