...
1. PICK THE ISSUE:
   Query GitHub for open issues labeled with `agent-ready-gemini` (e.g. `gh issue list --state open --label "agent-ready-gemini" --json number,title`). NEVER pick up issues labeled only `agent-ready` — those belong to the Claude routine.
   ...
   *** ROBUST DEDUP — this is critical; we already had a duplicate-PR incident (#207 AND #208 were both filed for issue #148 because a weak check missed the first PR). An issue is ELIGIBLE only if it is OPEN and has NO open PR and NO remote branch already targeting it. Do NOT rely on GitHub's formal 'linked PR' relationship — that only exists when a PR body literally says 'Closes #N', so a PR that merely mentions the issue will be MISSED. Instead check ALL of: `gh pr list --state open --search "<n>"`; `gh pr list --state open --json number,title,body,headRefName` then grep title/body/branch for the issue number; and `git ls-remote --heads origin '*issue-<n>-*'`. ANCHOR YOUR MATCHES — raw substring matching on the number is a bug in the other direction: `*issue-15*` also matches `issue-150`/`issue-155` branches and would wrongly mark issue #15 ineligible forever. Match branch names against `issue-<n>-` (trailing dash) and grep PR text for `#<n>` followed by a non-digit (e.g. `grep -E '#15([^0-9]|$)'`); when `gh pr list --search "<n>"` returns hits, confirm each hit actually refers to issue <n> before counting it. If ANY of these shows existing work for the issue (by ANY agent, Claude or Gemini), treat it as INELIGIBLE and move to the next. (Farm these lookups to a subagent.) ***
...
5. Otherwise implement the SMALLEST change that closes the issue. Respect god-file warnings in CLAUDE.md (recipe-lanes/app/lanes/page.tsx, recipe-lanes/lib/data-service.ts, recipe-lanes/components/recipe-lanes/react-flow-diagram.tsx). You make design decisions; delegate boilerplate edits to a subagent if useful.
...
9. Commit, push the branch, open a PR against main WITH the label `agent:gemini` (`gh pr create --base main --label "agent:gemini" ...`). The PR body MUST include `Closes #<n>` so GitHub formally links it (this also keeps future dedup reliable). Then POST A COMMENT on the issue linking the PR (progress log). Every comment you post must end with the marker line `<!-- gemini-agent -->`.
10. WAIT FOR CI — REQUIRED. Opening the PR is NOT 'done'. Block on a SINGLE watch call (`gh pr checks <pr> --watch`; if gh is missing, poll the REST API sparingly — never a tight loop). Have a subagent read any failing logs so you don't ingest them. VERIFY THE TEST JOBS ACTUALLY RAN: `fast-checks`, `integration`, `e2e` must EXECUTE, not skip. If they skip as docs-only/no-code, that is a RED FLAG — investigate. Done = required checks GREEN with tests having executed. Note: an e2e failure that is clearly an unrelated pre-existing flake (e.g. HuggingFace embedding-model download in icon search) is NOT your regression — say so but do not claim you fixed it. If RED for a real reason: fix, push, wait again. HANG-GUARD: if a required check stays queued a long time without progressing, report 'CI stuck pending: <check>' and stop. If NO CI checks appear on the PR at all within ~5 minutes, report 'CI did not trigger — the agent GitHub App (AGENT_APP_ID/AGENT_APP_PRIVATE_KEY) or AGENT_GH_PAT fallback may be missing/expired' and stop; do not claim success.
