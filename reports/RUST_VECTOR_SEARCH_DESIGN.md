...
**Work Updates:**
- **Git Worktree:** Created off broken `feature/icon-search-pipeline` commit to minimize diff conflicts during migration.
- **Backend Build (`[bb202d2]`):**
    - Initialized new Rust binary project (`rust-vector-search`).
    - Set up an `axum` HTTP server handling `/search` and `/embed` operations natively.
    - Integrated `fastembed v4` with `AllMiniLML6V2`.
    - **Problem Encountered:** When using the bleeding-edge `fastembed v5.13`, ONNX runtime (`ort`) threw C++ linker errors (`__isoc23_strtoull` / `__cxa_call_terminate`) during test compilation due to glibc mismatching between precompiled binaries and the local system.
    - **Solution:** Downgraded `fastembed` to `v4.9`, resolving the compiler issues immediately without sacrificing performance.
    - **Testing:** Added multithread-safe locking around the embedder (`Mutex<TextEmbedding>`) to prevent Tokio race conditions. Created `test_embed_only` and `test_with_index` cases.
- **Index Scripting (`[bb202d2]`):**
    - Built a robust node script `pull-db.ts` utilizing `firebase-admin` that dynamically accepts `staging` or `prod` as CLI arguments.
    - Safely parses and downloads only the necessary `icon_id`s and float arrays from live Firestore, dropping extraneous metadata as outlined in the schema refactor.
    - Set up `./boop` testing script (a quick `curl` utility to ping `localhost:8080/search`). Tested and validated it returns 12 exact icon IDs in parallel.
- **Client Side React Orchestration (`[42acc6f]`):**
    - Created `icon-search-strategy.ts` defining the unified `IconSearchStrategy` interface alongside `RustCloudRunStrategy`, `BrowserLocalStrategy`, and `LegacyVertexStrategy`.
    - Created the `useHybridIconSearch.ts` Next.js hook that accepts queries, hits the active Strategy (0-50ms), fires state to trigger an instant React render, and then natively stages the `Slow Pass` (Firestore delta) utilizing the embedding payload successfully generated by the Rust proxy.
